...
def detect_license_plate():
    ...
    for contour in contours:
        ...
        if 4<=len(approx)<=9:
            ...
            lcd_clear()  #Izbrisi sadrzaj displeja prije postavljanja novih poruka
            ...
            if extracted_text and len(extracted_text) in [7, 8]:
                ...
                #Provjeri da li je tablica u bazi podataka
                ...
                mydb.close()
                time.sleep(3)
                lcd_clear()  #Izbrisi sadrzaj displeja prije nastavka
...
try:
    while True:
        ...
        while consistent_distance_count<10:
            distance=measure_distance()
            print(f"Provjeravanje udaljenosti: {distance:.1f} cm")
            if 10<=distance<=40:
                consistent_distance_count+=1
            else:
                consistent_distance_count=0
            time.sleep(0.5)
        print("Detekcija tablice pocinje...")
        detect_license_plate()
        time.sleep(3) #Pauza prije ponovne provjere udaljenosti objekta
